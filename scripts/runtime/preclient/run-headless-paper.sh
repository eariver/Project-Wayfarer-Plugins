...
candidate_version="${WAYFARER_HEADLESS_VERSION:-0.0.2-headless.1}"
...
curl --fail --silent --show-error --location \
  --user-agent "Project-Wayfarer-Plugins-preclient/0.0.2 (https://github.com/eariver/Project-Wayfarer-Plugins)" \
  "$paper_url" \
  --output "$paper_jar"
...
mapfile -t core_jars < <(
  find "$repository_root/plugins/wayfarer-core/build/libs" -maxdepth 1 -type f \
    -name "wayfarer-core-${candidate_version}.jar"
)
mapfile -t probe_jars < <(
  find "$repository_root/testkit/headless-paper/build/libs" -maxdepth 1 -type f \
    -name "wayfarer-preclient-probe-${candidate_version}.jar"
)
mapfile -t main_jars < <(
  find "$repository_root/plugins/wayfarer-main/build/libs" -maxdepth 1 -type f \
    -name "wayfarer-main-${candidate_version}.jar"
)
mapfile -t frontier_jars < <(
  find "$repository_root/plugins/wayfarer-frontier/build/libs" -maxdepth 1 -type f \
    -name "wayfarer-frontier-${candidate_version}.jar"
)
mapfile -t fixture_jars < <(
  find "$repository_root/testkit/headless-waymark-fixture/build/libs" -maxdepth 1 -type f \
    -name "wayfarer-preclient-waymark-fixture-${candidate_version}.jar"
)
test "${#core_jars[@]}" -eq 1
test "${#probe_jars[@]}" -eq 1
test "${#main_jars[@]}" -eq 1
test "${#frontier_jars[@]}" -eq 1
...
frontier_sha256="$(sha256sum "$frontier_jar" | awk '{print $1}')"
...
CONFIG
  ...
  cp "$frontier_jar" \
    "$server_root/plugins/Wayfarer_Frontier-${candidate_version}.jar"
  if [[ "$include_probe" == "yes" ]]; then
    cp "$probe_jar" \
      "$server_root/plugins/Wayfarer_Preclient_Probe-${candidate_version}.jar"
  fi
  if [[ "$include_fixture" == "yes" ]]; then
    cp "$fixture_jar" \
      "$server_root/plugins/Vault_Test_Fixture-${candidate_version}.jar"
  fi
...
assert_runtime_classloading() {
  local log_file="$1"
  grep -Fq "WAYFARER_FIXTURE_VAULT: REGISTERED provider=RedisEconomy" "$log_file"
  grep -Fq "WAYFARER_FIXTURE_VAULT: CALL getBalance thread=Server thread" \
    "$log_file"
  grep -Fq "WAYFARER_FIXTURE_VAULT: CALL withdrawPlayer thread=Server thread" \
    "$log_file"
  if grep -Eq \
    "NoClassDefFoundError|ClassNotFoundException|Error occurred while enabling (Wayfarer_Core|Wayfarer_Preclient_Probe)" \
    "$log_file"; then
    echo "Core or probe classloading/enable failure detected"
    return 1
  fi
}
...
wait_for_log "$baseline_first_log" \
  "Wayfarer_Main runtime enabled; module migration and persistence are UP." 60
wait_for_log "$baseline_first_log" \
  "Wayfarer_Frontier runtime enabled; module migration and persistence are UP." 60
...
send_command "wayfarer-main status"
send_command "wayfarer-frontier status"
...
grep -Fq "Wayfarer_Core $candidate_version | server=wayfarer-preclient | config=1 | lifecycle=ENABLED" "$baseline_first_log"
...
grep -Fq "Wayfarer Main: ENABLED | config=1" "$baseline_first_log"
grep -Fq "Wayfarer Frontier: ENABLED | config=1 | world=frontier_iris" \
  "$baseline_first_log"
assert_runtime_classloading "$baseline_first_log"
...
wait_for_log "$baseline_restart_log" \
  "Wayfarer_Main runtime enabled; module migration and persistence are UP." 60
wait_for_log "$baseline_restart_log" \
  "Wayfarer_Frontier runtime enabled; module migration and persistence are UP." 60
stop_server
...
# Vault API remains resolvable while its Economy service is absent; Core fails closed.
...
sleep 2
stop_server
grep -Fq "WAYFARER_FIXTURE_VAULT: API_ONLY provider=unavailable" "$outage_log"
grep -Fq "Transaction: DOWN" "$outage_log"
if grep -Eq \
  "NoClassDefFoundError|ClassNotFoundException|Error occurred while enabling (Wayfarer_Core|Wayfarer_Preclient_Probe)" \
  "$outage_log"; then
  echo "Provider-absent classloading/enable failure detected"
  exit 1
fi
...
java=25
candidate_core_version=$candidate_version
candidate_core_sha256=$core_sha256
candidate_main_sha256=$main_sha256
candidate_frontier_sha256=$frontier_sha256
...
vault_api_classloader=pass
vault_provider_discovery=pass
provider_absent_fail_closed=pass
queue_rejection=pass
accepted_work_drain=pass
shutdown_timeout_non_clean=pass
provider=TEST_ONLY_VAULT_REDISECONOMY_FIXTURE
vault_effect_lookup=unavailable_by_contract
