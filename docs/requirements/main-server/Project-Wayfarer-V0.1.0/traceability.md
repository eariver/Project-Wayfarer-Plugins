...
| LIF-010 | Services unregister | 8.1 | `BukkitServicePublisher`; reverse cleanup | Disable/idempotency tests | V0.0.1-alpha.1 | alpha.1 runtime evidence | Runtime test passed | Clean disable and restart showed no duplicate registration |
| LIF-011 | Executor lifecycle | 8.1 | `ManagedExecutor`; explicit shutdown result | Graceful/forced/incomplete/interrupted tests | V0.0.1-alpha.1 / alpha.3 | alpha.1 runtime evidence | In progress | Graceful, forced, and incomplete observed; interrupted runtime and queue/backpressure remain pending |
...
| LIF-013 | Clean disable | 8.1 | `LifecycleCoordinator`; shutdown-result health | Lifecycle/runtime health tests | V0.0.1-alpha.1 | alpha.1 runtime evidence | Runtime test passed | Clean disable and restart observed; incomplete remains `DOWN` |
| LIF-014 | Timeout-bounded flush | 8.1 | Two-phase executor shutdown timeout | Graceful/forced/incomplete/interrupt tests | V0.0.1-alpha.1 | alpha.1 runtime evidence | Runtime test passed | Graceful and forced timeout phases observed |
