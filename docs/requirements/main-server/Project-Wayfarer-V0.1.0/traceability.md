...
| LIF-006 | Redis foundation | 8.1 | Internal `RedisRuntime`; command/Pub/Sub lifecycle | Isolated Redis integration test | alpha.3 | Pending | In progress | Cache/lock/message/idempotency foundations implemented; updated-head CI pending |
| LIF-007 | Redis health | 8.1 | Connection listeners and operation/close health | Outage/reconnect/close test | alpha.3 | Pending | In progress | UNKNOWN/UP/DOWN/recovery/DISABLED paths implemented; updated-head CI pending |
...
| CFG-004 | Redis connection settings | 8.3 | Typed URI reference, connect/operation timeouts, TTL/lease bounds, key prefix | Validation/missing-secret and isolated connection tests | alpha.3 | alpha.1 runtime evidence | In progress | Backward defaults and bounds passed locally; isolated CI pending |
...
| RED-001 | Redis cache foundation | 8.8 | Namespaced schema envelope, bounded TTL, `cacheAside` | Integration/cache fallback test | alpha.3 | Pending | In progress | MariaDB loader remains authority; updated-head CI pending |
...
| RED-003 | Redis pub/sub and invalidation | 8.8 | Versioned channel/envelope and immutable listener boundary | Integration test | alpha.3 | Pending | In progress | Self/duplicate/invalid policies implemented; CI pending |
...
| RED-005 | Redis idempotency assistance | 8.8 | Bounded `SET NX PX` advisory hint | Idempotency test | alpha.3/4 | Pending | In progress | MariaDB unique key documented as final authority; CI pending |
...
| RED-009 | Redis reconnect | 8.8, 10.2 | Lettuce reconnect listeners plus successful-operation recovery | Pause/reconnect test | alpha.3 | Pending | In progress | Updated-head isolated CI pending |
...
| TASK-010 | Graceful shutdown | 8.9 | Persistence/Redis gates and bounded `ManagedExecutor` | Queued accepted-work drain and shutdown tests | alpha.2/3 | Pending | In progress | Accepted queue drains after intake closes; Redis CI/runtime pending |
...
| TASK-012 | No main-thread Redis I/O | 6.4, 8.9 | `RedisRuntime` pre-dispatch `ThreadContext` guard | Detection test | alpha.3 | Pending | In progress | Integration source compiled; CI pending |
...
| TST-003 | Redis integration | 10.1 | `redisIntegrationTest` using authoritative `redis:8-alpine` | Testcontainers | alpha.3 | Pending | In progress | Six cases compile; updated-head CI pending |
...
| TST-013 | Main-thread I/O and callback tests | 10.2 | JDBC/Redis guards, immutable player snapshot, task bridge | Automated/runtime | alpha.2/3 | Pending | In progress | Local unit paths passed; Redis CI and Paper runtime pending |
