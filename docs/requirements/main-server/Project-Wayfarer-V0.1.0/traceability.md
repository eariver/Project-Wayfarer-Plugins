...
| LIF-013 | Clean disable | 8.1 | `LifecycleCoordinator`; Identity close/finalize; persistence drain; shutdown-result health | Lifecycle, queued-drain, identity-finalization, and runtime health tests | V0.0.1-alpha.1 / alpha.2 | alpha.1 runtime evidence | Runtime test passed | Identity becomes clean only after accepted work and database drain complete; Paper persistence-drain evidence remains pending |
| LIF-014 | Timeout-bounded flush | 8.1 | Bounded Identity finalization and persistence drain plus two-phase executor shutdown | Identity/persistence timeout and interrupt plus executor shutdown tests | V0.0.1-alpha.1 / alpha.2 | alpha.1 runtime evidence | Runtime test passed | Identity/persistence timeout/interruption are `DOWN` and non-clean; executor graceful and forced timeout phases observed |
