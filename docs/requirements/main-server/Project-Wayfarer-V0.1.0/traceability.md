...
| LIF-013 | Clean disable | 8.1 | `LifecycleCoordinator`; persistence drain; shutdown-result health | Lifecycle, queued-drain, and runtime health tests | V0.0.1-alpha.1 / alpha.2 | alpha.1 runtime evidence | Runtime test passed | Automated drain completes before pool close; Paper persistence-drain evidence remains pending |
| LIF-014 | Timeout-bounded flush | 8.1 | Bounded persistence drain plus two-phase executor shutdown | Drain timeout/interrupt and executor shutdown tests | V0.0.1-alpha.1 / alpha.2 | alpha.1 runtime evidence | Runtime test passed | Persistence timeout/interruption are `DOWN` and non-clean; executor graceful and forced timeout phases observed |
